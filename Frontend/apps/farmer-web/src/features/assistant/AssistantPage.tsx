...
export function AssistantPage() {
  ...
  const allMessages: AssistantMessage[] = isActiveThread
    ...
    : []; // Non-active threads would load from API; fixture only has one thread
          ...
          <div className="flex flex-col gap-1 overflow-y-auto">
            {threadList.map((th) => (
              <button
                ...
                className={`text-start rounded-[var(--hv-radius-md)] px-3 py-2 text-[var(--hv-text-sm)] transition-colors ${
